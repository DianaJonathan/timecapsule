  ...
  public load(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        ...
        new Promise<void>((res, rej) => {
          ...
          script.onload = () => {
            if (!isFhevmWindowType(window, this._trace)) {
              ...
              rej(
                new Error(
                  `RelayerSDKLoader: Relayer SDK loaded from ${src}, but window.relayerSDK is invalid.`
                )
              );
              return;
            }
          ...
          script.onerror = () => {
            ...
            rej(new Error(`RelayerSDKLoader: Failed to load from ${src}`));
          };
      ...
      loadFrom(SDK_CDN_URL)
        .then(resolve)
        .catch(() => {
          // Fallback to local UMD file
          loadFrom(SDK_LOCAL_URL).then(resolve).catch(reject);
        });
    });
  }
}
